...
LABEL org.opencontainers.image.title="VNC Viewer" \
    ...
    com.docker.extension.detailed-description="<h1 id=docker-extension-vnc>Docker extension VNC</h1><p>A VNC Viewer extension for Docker Desktop.<h2 id=table-of-contents>Table of Contents</h2><ul><li><a href=#description>Description</a><li><a href=#manual-installation>Manual Installation</a><li><a href=#usage>Usage</a><ul><li><a href=#the-example-container>Example Container</a><li><a href=#sessions>Sessions</a><ul><li><a href=#docker-container>Docker Container</a><li><a href=#remote-host>Remote Host</a></ul></ul></ul><h2 id=description>Description</h2><p>This Docker Desktop extension is for connecting to Docker Containers or a Remote Host which have a <a href=https://en.wikipedia.org/wiki/VNC>VNC</a> Server running.<p>The extension creates a Docker Container as a <a href=https://hub.docker.com/r/pgmystery/proxy_vnc>proxy</a> for connecting to the target VNC Server and shows the view inside of Docker Desktop.<p>If the connection is to a Docker Container, then the extension creates a Docker Network and adds the proxy and the target container to the Network. Only this way, the proxy can access the VNC server container. After disconnect, it deletes the proxy container and removes the VNC server Container from the Network.<p>It uses <a href=https://github.com/novnc/noVNC>noVNC</a> as the JavaScript client package.<p>It also comes with an example VNC Server as a docker container. So you can test things out very easily.<h2 id=manual-installation>Manual Installation</h2><p>You can install the extension by using the command:<pre><code class=lang-shell>docker <span class=hljs-keyword>extension</span> install pgmystery/docker-<span class=hljs-keyword>extension</span>-vnc:<span class=hljs-number>1.0</span>.<span class=hljs-number>0</span></code></pre><h2 id=usage>Usage</h2><h3 id=the-example-container>The example Container</h3><p>You can right start by seeing the possibility of this extension if you just click the button \"Try example container\". It will download an <a href=https://hub.docker.com/r/pgmystery/ubuntu_vnc>example Docker Image</a> with a vnc server and a Desktop installed. It will also then automatically creates the Container and a connection to the container in the extension will be accomplished.<p><img alt=Dashboard src=https://github.com/pgmystery/docker-extension-vnc/blob/backend_sessions/docs/imgs/screenshot4.png><h3 id=sessions>Sessions</h3><p><strong>⚠️IMPORTANT: After upgrading the extension to a new version, all Sessions getting deleted!⚠️</strong><p>To switch the target and the connection, you can create Sessions. The Session are saved in the Backend Docker Container in a SQLite Database.<p>To create a new Session, just click on the plus icon button. Now you can enter the Session name, the connection type with data and optional the credentials.<p><img alt=\"Edit Session\"src=https://github.com/pgmystery/docker-extension-vnc/blob/backend_sessions/docs/imgs/screenshot6.png><p>This extension support 2 different kind of connections.<ul><li><a href=#docker-container>Docker Container</a><li><a href=#remote-host>Remote Host</a></ul><h4 id=docker-container>Docker Container</h4><ol><li>Select the connection type \"Docker Container\".<li>Select the Container from the list (if the container is not showing up, you can also manually insert the Container Name or ID).<li>Enter or select the port on which the vnc server is listening on (it is an internal port, it don't need to be exposed to the host).<li>Save the Session.<li>Click on \"Connect\".</ol><p><img alt=\"Webbrowser on a linux desktop\"src=https://github.com/pgmystery/docker-extension-vnc/blob/backend_sessions/docs/imgs/screenshot1.png><h4 id=remote-host>Remote Host</h4><ol><li>Select the connection type \"Remote Host\".<li>Enter the IP of the Host you have access to.<li>Type in the port of the VNC Server on the Remote Host.<li>Save the Session.<li>Click on \"Connect\".</ol><p><img alt=\"Remote Host connection\"src=https://github.com/pgmystery/docker-extension-vnc/blob/backend_sessions/docs/imgs/screenshot5.png>" \
